...
def _category_weight_breakdown(requirements: list[JDRequirement]) -> list[dict]:
    ...
    for req in requirements:
        totals[getattr(req.category, 'value', str(req.category))] += max(0, int(req.weight or 0))
    ...
    with st.container(border=True):
        if duplicate_groups:
            ...
            for number, group in enumerate(duplicate_groups, start=1):
                ...
                for req in group:
                    category = getattr(req.category, 'value', str(req.category)) if hasattr(req.category, "value") else str(req.category)
                    st.markdown(
        ...
        if bad_gates:
            ...
            for req in bad_gates:
                category = getattr(req.category, 'value', str(req.category)) if hasattr(req.category, "value") else str(req.category)
                st.markdown(
                    f"- `{req.requirement_id}` **[{category}]**: {req.name}"
                )
...
def _row_from_requirement(req, display_id, requirement_name=None, group_display=None, operator_display=None):
    return {
        "ID": display_id,
        "Category": getattr(req.category, 'value', str(req.category)),
...
def _render_bulk_controls(analysis: JDAnalysis, tier: str):
    ...
    categories = sorted({getattr(r.category, 'value', str(r.category)) for r in tier_requirements})
    if not categories:
        return
    ...
    with c4:
        if st.button("Apply to category", key=f"bulk_apply_{tier}"):
            ...
            for req in analysis.requirements:
                if _tier(req) != tier or getattr(req.category, 'value', str(req.category)) != category:
                    continue
